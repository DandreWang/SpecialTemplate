...
<script>
  ...
  // 工具
  const { jumpQuotation, shareFun, getAppVsn, mobileEnv: { isFutures, ios }, setTitle } = hybird
  ...
  export default {
    ...
    methods: {
      ...
      async getHqData(data) {
        ...
        // 声明
        const { getStaticData, fmtGnList, fmtList, fmtTitle } = this
        // 请求
      ...
      // 格式化页面信息
      async reload(n) {
        const { tit, subTit, itemDbId, btnDbId, mainClsId, shareConf: { shareDesc } = {}, gwIds } = n
        // 空的则返回
        if (!tit) return
        // 更新
        Object.assign(this, {
          tit, subTit, itemDbId, btnDbId, mainClsId, shareDesc, gwIds
        })
        // 行情列表
