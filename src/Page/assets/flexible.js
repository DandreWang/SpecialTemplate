...
;(function (doc, win) {
  ...
  const isIpad = /(?:iPad|PlayBook)/.test(u) || (isAndroid && !/(?:Mobile)/.test(u) || /(?:Firefox)/.test(u) && /(?:Tablet)/.test(u))
  // 文档对象
  ...
  // html对象
  const htmlDom = doc.getElementsByTagName('html')[0]
  // 重置函数
  const recalc = _ => {
    // 获取原窗口的高度、宽度
    ...
    // const clientWidth = originalWidth > 600 ? 600 : originalWidth
    const fz = 50 * originalWidth / (isIpad ? (originalWidth > 500 ? 750 : 375) : 375) + 'px'
    docEl.style.fontSize = fz
    htmlDom.style.fontSize = fz
    htmlDom.setAttribute('style', 'font-size: ' + fz)
  }
